    ...
    constructor(private history: History.History, routes: Route[], basename: string) {
        ...
        routes.forEach(route => {
            crossroads.addRoute(route.url, (requestParams: any) => {
                if (route.canActivate==null || route.canActivate(this.history, route)){
        ...
        this.disposeHistory = history.listen(location => crossroads.parse(location.hash));
        this.clickEventListener = (evt: Event) => {
        ...
        crossroads.parse(history.location.hash);
    }
...
export interface Route {
    url?: string;
    params?: any;
    canActivate?: (history: History.History, callingRoute : Route) => boolean;
}
